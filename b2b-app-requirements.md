...
- **Overlay, never recompute.** The storefront engine overlays our tier delta *on top of the price Shopify already rendered* (the native catalog price), so it cannot double-discount or drift from the Function. Percent tiers are applied client-side against `shop.money_format`-parsed prices (theme-agnostic, no extra network). Amount tiers and per-product exclusions (`b2b_only`, compare-at/sale prices) are resolved against an authoritative batched App Proxy `/tier-prices` map (KV-cached, reuses the `packages/shared` pricing module so it cannot drift).
