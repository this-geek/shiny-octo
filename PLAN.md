...
- [ ] **P0** Worker-hosted dealer portal via App Proxy (replaces the `customer-account.page.render` extension, which kept tripping Shopify's "cannot combine targets" validator). *(Steps 1–2 done: `apps/worker/src/routes/portal.ts` — entry handler at `/proxy/portal` (reached via `<shop>/apps/b2b/portal`) plus JSON API subroutes at `/proxy/portal/api/{assets/list, assets/download/:id, profile, tour-status, tour-dismiss}`. Mounted under the existing `appProxyRouter` so each hit is HMAC-verified by Shopify; resolves buyer via `resolveBuyerByCustomerId`; renders sign-in / not-wholesale / error / shell pages with CSP + `noindex` + `no-store`. API routes reuse `lib/asset-serve.ts`, `lib/company-profile.ts`, `lib/tour-state.ts` unchanged so behaviour cannot drift from the customer-account router. No session cookie — every XHR through the proxy is re-signed by Shopify on the fly, so HMAC is the auth. Next steps: ship the SPA bundle, convert the customer-account extension to a single-target link block, then delete `routes/customer-account.ts` + the orphaned page-extension code.)*
