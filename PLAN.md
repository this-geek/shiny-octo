...
- [ ] **P0** Worker-hosted dealer portal via App Proxy (replaces the `customer-account.page.render` extension, which kept tripping Shopify's "cannot combine targets" validator). *(Step 1 in progress: `apps/worker/src/routes/portal.ts` — entry handler at `/proxy/portal` (reached via `<shop>/apps/b2b/portal`) mounted under the existing `appProxyRouter` so each hit is HMAC-verified by Shopify; resolves buyer via `resolveBuyerByCustomerId`; renders sign-in / not-wholesale / error / shell pages with CSP + `noindex` + `no-store`. No session cookie — every XHR through the proxy is re-signed by Shopify on the fly, so HMAC is the auth. Next steps: port `/customer-account/*` JSON routes under `/proxy/portal/api/*`, ship the SPA bundle, convert the customer-account extension to a single-target link block, then delete the orphaned page-extension code.)*
