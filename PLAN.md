...
- [ ] **P1** App Proxy `/tier-prices?variant_ids=…` authoritative batch map (KV-cached, reuses `packages/shared`) for amount tiers + per-product exclusions (`b2b_only`, compare-at/sale). *(v2.)*
- [ ] **P1** Native-catalog reconciliation: overlay no-ops when the tier delta is zero (tier mapped 1:1 to a native catalog) and on Plus. *(v2. v1 already no-ops at 0% / when no tier resolves.)*
- [~] **P0** Force-login guidance: go-live checklist item + one-time admin banner ("enable require-login for site-wide pricing"). *(v1 ships the guidance inline as an info `Banner` in Settings → Price display when the toggle is on. The onboarding go-live checklist item + a dedicated dismissible admin banner are still to wire.)*
- [~] **P0** Parity tests + Playwright site-wide matrix (collections / search / home on Dawn + Horizon + Impulse + Prestige) holding CLS ≤ 0.1 / INP ≤ 200ms. *(v1 adds the money-parse/format unit battery; the cross-target discount parity is already covered by `b2b-price.test.js`. Playwright site-wide matrix + Web Vitals budget still to add — needs the captured listing-page fixtures per theme.)*
