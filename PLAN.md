...
- [ ] **P0** Worker-hosted dealer portal via App Proxy (replaces the `customer-account.page.render` extension, which kept tripping Shopify's "cannot combine targets" validator). *(Steps 1–3 done: `apps/worker/src/routes/portal.ts` — entry handler at `/proxy/portal`, JSON API subroutes at `/proxy/portal/api/*`, and a vanilla-JS SPA bundle served from `/proxy/portal/static/app.{js,css}` (string-exported from `routes/portal-assets/` to skip a build step). Shell page links the external assets, embeds boot JSON with `proxy_base` derived from Shopify's `path_prefix` query so URLs work across merchant proxy subpaths. CSP tightened to `script-src 'self'; style-src 'self'` (no inline). All routes mounted under `appProxyRouter` so every hit — entry, API, and static — is HMAC-verified by Shopify. API routes reuse `lib/asset-serve.ts`, `lib/company-profile.ts`, `lib/tour-state.ts` unchanged so behaviour cannot drift from the customer-account router. Next steps: convert the customer-account extension to a single-target link block, then delete `routes/customer-account.ts` + the orphaned page-extension code.)*
