...
describe('cart-transform Function', () => {
  it('returns empty operations when shop.metafield.value === "true" (Plus)', () => {
    const input: FunctionInput = { shop: { metafield: { value: 'true' } } };
    expect(run(input)).toEqual({ operations: [] });
  });

  it('returns empty operations when shop.metafield is null (non-Plus, Phase 1A scaffold)', () => {
    const input: FunctionInput = { shop: { metafield: null } };
    expect(run(input)).toEqual({ operations: [] });
  });

  it('returns empty operations when shop.metafield.value === "false" (non-Plus, Phase 1A scaffold)', () => {
